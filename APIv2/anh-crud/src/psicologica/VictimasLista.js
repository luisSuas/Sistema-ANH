...
function normalizeAreaSlug(area) {
  if (area == null) return "psicologica";
  const s = String(area).trim().toLowerCase();
  if (/^\d+$/.test(s)) {
    const n = parseInt(s, 10);
    return ({ 1: "social", 2: "legal", 3: "medica", 4: "psicologica", 5: "albergue" }[n]) || "psicologica";
  }
  const map = {
...
function VictimasLista() {
  ...
  // ====== FORZAR AREA PSICOLOGICA ======
  const FORCED_AREA = "psicologica";
