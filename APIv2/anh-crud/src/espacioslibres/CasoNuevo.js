...
/* ✅ Helpers de área para que funcione igual en Médica/Social/Legal/... y ahora Espacios Libres */
...
export default function CasoNuevo() {
  ...
  // ✅ Base de rutas según área actual (soporta Espacios Libres)
  const areaSlug = useMemo(
    () => normalizeAreaSlug(inferAreaFromPath(location.pathname) || areaIdFromToken),
    [location.pathname, areaIdFromToken]
  );
  ...
  // ✅ Antes estaba fijo a /medica
  const irDetalle = () => { if (casoId) nav(`${areaBase}/casos/${casoId}`); };
  ...
  return (
    <div className="cn-wrap">
      <div className="cn-header">
        <div>
          {/* ✅ Antes estaba fijo a /medica */}
          <Link to={areaBase} className="cn-back">
            ← Volver
          </Link>
          <h2>Nuevo proceso</h2>
          ...
          <Row>
            <Col>
              ...
              <small className="cn-help">
                ¿No la sabes? Ve a{" "}
                {/* ✅ Antes estaba fijo a /medica/victimas */}
                <Link to={`${areaBase}/victimas`}>Sobrevivientes</Link>{" "}
                y copia el ID.
              </small>
            </Col>
            <Col>
