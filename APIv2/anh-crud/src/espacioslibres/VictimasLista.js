...
function normalizeAreaSlug(area) {
  if (area == null) return "espacioslibres";
  const s = String(area).trim().toLowerCase();
  if (/^\d+$/.test(s)) {
    const n = parseInt(s, 10);
    return ({ 1: "social", 2: "legal", 3: "medica", 4: "psicologica", 5: "albergue" }[n]) || "espacioslibres";
  }
  const map = {
...
function VictimasLista() {
  ...
  // ====== FORZAR AREA ESPACIOS LIBRES ======
  const FORCED_AREA = "espacioslibres";
  ...
  const goBackHome = () => {
    navigate(areaBase); // /espacioslibres
  };
